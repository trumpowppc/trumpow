...
# TRMP TODO Change these urls to trumpow.meme and add a depends subdomain which will do what this does
# Dependencies
...
Standalone script to perform the gitian build of Trumpow Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/trumpowppc/trumpow
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/trumpow-binaries/${VERSION}/ \;
}
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/trumpow-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean trumpow git directory because of old caching
    if [ -d inputs/trumpow/ ]; then
        echo "Cleaning Trumpow directory..."
        rm -rf inputs/trumpow/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit trumpow="$COMMIT" --url trumpow="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
