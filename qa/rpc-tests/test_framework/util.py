...
def initialize_datadir(dirname, n):
    ...
    with open(os.path.join(datadir, "trumpow.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def wait_for_bitcoind_start(process, url, i):
    ...
    while True:
        if process.poll() is not None:
            raise Exception('trumpowd exited with status %i during initialization' % process.returncode)
        try:
