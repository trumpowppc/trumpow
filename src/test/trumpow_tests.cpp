...
BOOST_FIXTURE_TEST_SUITE(trumpow_tests, TestingSetup)

/**
 * Return the maximum block reward (without fees) at a given height.
 */
...
/**
 * Return the *minimum possible* block reward (without fees) at a given height.
 * For pre-launch/testing, blocks may be empty/mined for zero reward.
 */
...
BOOST_AUTO_TEST_CASE(subsidy_test)
{
    ...
    for (int nHeight = 0; nHeight < 1050000; nHeight++) {
        const Consensus::Params& params = mainParams.GetConsensus(nHeight);
        CAmount nSubsidy = GetTrumpowBlockSubsidy(nHeight, params, prevHash);
    ...
    CAmount nConstantSubsidy = GetTrumpowBlockSubsidy(1050000, mainParams.GetConsensus(1050000), prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);

    nConstantSubsidy = GetTrumpowBlockSubsidy(1250000, mainParams.GetConsensus(1250000), prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_difficulty_limit)
{
    ...
    BOOST_CHECK_EQUAL(CalculateTrumpowNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1e0fffff);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_pre_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculateTrumpowNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1c271b09);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield)
{
    ...
    pindexLast.nHeight = 145000;//TRMP TODO Magic number
    pindexLast.nTime = 1395094679;
    pindexLast.nBits = 0x1b499dfd;
    BOOST_CHECK_EQUAL(CalculateTrumpowNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b671062);
}

BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_upper)
{
    SelectParams(CBaseChainParams::MAIN);
    const Consensus::Params& params = Params().GetConsensus(1000);//TRMP TODO Magic number
    ...
    BOOST_CHECK_EQUAL(CalculateTrumpowNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b4e56b3);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_lower)
{
    ...
    BOOST_CHECK_EQUAL(CalculateTrumpowNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b335358);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_rounding)
{
    ...
    BOOST_CHECK_EQUAL(CalculateTrumpowNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b6558a4);
}
...
BOOST_AUTO_TEST_CASE(hardfork_parameters)
{
    ...
    const Consensus::Params& digishieldParams = Params().GetConsensus(1000);//TRMP TODO Magic number
    ...
    const Consensus::Params& digishieldParamsEnd = Params().GetConsensus(59999);
    ...
    const Consensus::Params& auxpowParams = Params().GetConsensus(60000);//TRMP TODO Magic number
    BOOST_CHECK_EQUAL(auxpowParams.nHeightEffective, 60000);//TRMP TODO Magic number
    ...
    const Consensus::Params& auxpowHighParams = Params().GetConsensus(1250000); // Arbitrary point after last hard-fork
