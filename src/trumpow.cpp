...
// Trumpow: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateTrumpowNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetTrumpowBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, uint256 prevHash)
{
    ...
    } else if (nHeight < (7 * consensusParams.nSubsidyHalvingInterval)) {
