...
const QString BITCOIN_IPC_PREFIX("trumpow:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (QFile::exists(arg)) // Filename
        {
            ...
            {
                if (request.getDetails().genesis() == "e81c430d51410470bcefcd8ecd4661bde27fb2ef14df54d48fb6c2cdaa0c1fde")
                {
                    SelectParams(CBaseChainParams::MAIN);
                }
                else if (request.getDetails().genesis() == "9f57e1ca1fac63513eb17a6d758294c17c9c2c16169a99dc3dfcc62c552474f6")
                {
                    SelectParams(CBaseChainParams::TESTNET);
                }
            }
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(ipcServerName)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start trumpow: click-to-pay handler"));
        }
        else {
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Trumpow address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
